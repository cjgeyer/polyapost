...
actually equality constraints (that is, they hold with equality for all points
in the constraint set), and \texttt{linearity} does one linear programming
problem for every inequality constraint (including the constraints that the
variables be nonnegative).  Call this the implied equality constraint problem.
In general, this step cannot be skipped because users may, either accidently
...
users may specify the problem so that   too much
...
aforementioned R package \texttt{polyapost}, version 1.3-1 finds the
